...
class Solution:
    def minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        ...
        while runLoop == True:
            # Manage profit
            # if counter <= len(customers) - 1:
            #     if customers[counter] > 4:
            #         profit = profit + ((4 * boardingCost) - ((counter + 1 if counter < 1 else counter) * runningCost))
            #     else:
            #         profit = profit + (((customers[counter]) * boardingCost) - ((counter + 1 if counter < 1 else counter) * runningCost))

            # manage customers
            if customers[counter] > 4:
                profit = profit + ((4 * boardingCost) - ((counter + 1 if counter < 1 else counter) * runningCost))
                if counter <= len(customers) - 2:
                    customers[counter + 1] = (customers[counter] - 4) + customers[counter + 1]
                else:
                    customers.append(customers[counter] - 4)
            else:
                profit = profit + (((customers[counter]) * boardingCost) - ((counter + 1 if counter < 1 else counter) * runningCost))

            # if customers[counter] > 4:
            #     print(customers[counter] - 4, counter, customers)
            # else:
            #     print(customers[counter], counter, customers)

            # return statements
            if customers[counter] <= 4 and counter == (len(customers) - 1) and profit > 1:
                runLoop = False
                return counter + 1
            if customers[counter] <= 4 and counter == (len(customers) - 1) and profit < 1:
                runLoop = False
            ...
            print(customers, customers[counter], counter, profit, len(customers))
...
# print(sl.minOperationsMaxProfit(
